...
class SECDataExtractor:
    ...
    SEC_CONCEPTS = [
        ...
        # Equity & Share Data
        SECConcept("CommonStockSharesOutstanding", units="shares"),
    ...
    def rate_limited_get(self, tag: str, ticker: str, taxonomy: str, units: str) -> pd.DataFrame:
        ...
        try:
            ...
            if "404" in error_str:
                ...
                self.logger.error(f"API call failed for {ticker}, concept {tag}: {error_str}")
                raise
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if 'value' in df.columns:
                        # Filter original filings and prepare the DataFrame
                        df = finagg.sec.api.filter_original_filings(df)
                        df = df.rename(columns={'end': 'filing_date', 'value': concept.tag})
                ...
                self.logger.error(f"Failed to retrieve {concept.tag} data for {ticker}: {str(e)}")
    ...
    def process_ticker(self, ticker: str) -> Tuple[str, str]:
        ...
        try:
            ...
            # Check if all concepts are 'N/A' or data is empty
            if df.empty or all(df[col].isnull().all() for col in df.columns if col != 'filing_date'):
            ...
            df = df.sort_values(['filing_date'] + [col for col in df.columns if col != 'filing_date'])
